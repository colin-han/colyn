...
colyn release --no-version-update  # push current branch only, no version bump or tag
colyn release patch --no-tag       # bump version and push branch, but skip the tag
