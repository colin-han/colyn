...
export function formatTodoTable(todos: TodoItem[], abbrMap?: Map<string, string>): string {
  ...
  const mkHeaderRow = (type: string, name: string, msg: string, created: string): string =>
    ' '.repeat(INDENT) +
    chalk.bold(padWidth(type, typeW)) + ' '.repeat(GAP) +
    chalk.bold(padWidth(name, nameW)) + ' '.repeat(GAP) +
    chalk.bold(truncWidth(msg, msgW)) + ' '.repeat(GAP) +
    chalk.bold(created);

  const mkDataRow = (type: string, name: string, msg: string, created: string): string =>
    ' '.repeat(INDENT) +
    chalk.gray(padWidth(type, typeW)) + ' '.repeat(GAP) +
    chalk.bold(padWidth(name, nameW)) + ' '.repeat(GAP) +
