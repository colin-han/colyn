...
提示：
  ...
  ? - Untracked (未被 git 跟踪的新文件)`,
    ...
    release: {
      ...
      lintSkipped: '已跳过 lint 检查（--no-build）',
      runningBuild: '运行 build...',
      buildSucceeded: '编译成功',
      buildSkipped: '已跳过 build 步骤（--no-build）',
      versionStepSkipped: '已跳过（--no-version-update）',
      tagSkipped: '已跳过打 tag',
...
主分支目录: {{path}}`,
      ...
      skippingBuild: '已跳过 lint 和 build 检查（--no-build）',
...
解决方法：
  重新安装：npm install -g colyn`,
    ...
    todo: {
      ...
      uncomplete: {
        description: '将已完成（done）的 Todo 回退为进行中状态',
        notFound: 'Todo "{{todoId}}" 不存在',
        notCompleted: 'Todo "{{todoId}}" 不是已完成（done）状态',
        noBranch: '当前不在 worktree 分支中，请指定 Todo ID',
        success: 'Todo "{{todoId}}" 已回退为进行中状态',
        usingCurrentBranch: '使用当前分支名: {{branch}}',
      },
      complete: {
        description: '将进行中的 Todo 标记为已完成（done）',
        notFound: 'Todo "{{todoId}}" 不存在',
        notPending: 'Todo "{{todoId}}" 不是进行中状态',
        noPending: '没有进行中的任务',
        selectTodo: '选择要完成的任务',
        success: 'Todo "{{todoId}}" 已标记为完成',
      },
      ...
      migrateLocal: {
        ...
        itemFailed: '迁移 "{{todoId}}" 失败：{{error}}',
        summary: '迁移完成：已迁移 {{migrated}} 项，跳过 {{skipped}} 项，失败 {{failed}} 项',
