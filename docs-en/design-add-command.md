...
graph TD
    ...
    ConfigEnv --> UpdateConfig[Update configuration file]
...
└── .colyn/              # Config directory (project identifier only)
```
...
### 7.2 Data Sources (No Config File)
...
**Rules**:
- Copy all content from main branch `.env.local`
- Update `PORT` to assigned port (main port + ID)
- Update `WORKTREE` to worktree ID
- Preserve all comments and other environment variables
...
3. **Run from Anywhere**: Auto-locate project root
4. **Complete Error Handling**: Detailed error messages and solutions
5. **Clear Success Feedback**: Show all key info and next step suggestions
6. **Technical Limitation Explanation**: Clearly explain auto-switch directory limitation and provide alternatives
